...
// #define RGBLIGHT_ANIMATIONS // disabling this removes about 9% of memory
// usage
...
#define IGNORE_MOD_TAP_INTERRUPT // requires holding both keys for the tapping
                                 // term
...
// the tapped key rather to hold a modifier key this allows us to use auto
// repeat for the tped key.

// the resolution can be specified in the same file (the default suggested is 4)
// might not need
#define ENCODER_RESOLUTION 2
#define ENCODER_DIRECTION_FLIP // they had this backwards and added this so we
                               // dont have to change all our code..

// a.llows media codes to properly register in macros and rotary encoder code
#define TAP_CODE_DELAY 10

// leader key
#define LEADER_TIMEOUT 300 // shorter timing because we ae using per key timing
#define LEADER_PER_KEY_TIMING // sets the timer for leader key chords to run on
                              // each key press rather than overall

// allows to use either side a the master. Loook at the documentation for
// information https://docs.qmk.fm/#/config_options?id=setting-handedness
#define EE_HANDS
...
/*
  If you define these options you will disable the associated feature, which can
  save on code size.
